<nav x-data="{ mobileOpen: false, scrolled: false }" @scroll.window="scrolled = (window.pageYOffset > 20)"
    ...
    <div class="flex items-center justify-between px-4 mx-auto max-w-7xl sm:px-6 lg:px-8">
        ...
        <a href="{{ route('user.dashboard') }}" class="flex items-center gap-2 group">
            <div class="h-10 w-auto">
                ...
                <span class="text-2xl font-bold tracking-tight hidden"
                    ...
                    Branding<span class="italic" :class="scrolled ? 'text-secondary' : 'text-white/90'">Aja</span><span
                ...
                <button @click="open = !open" class="flex items-center gap-3 transition-opacity hover:opacity-80">
                    <div class="hidden text-right xl:block">
                        <p class="text-sm font-bold transition-colors"
                            :class="scrolled ? 'text-gray-800' : 'text-white'">Hi,
                            {{ $user->name ?? Auth::user()->name ?? 'Username' }}</p>
                    </div>
                    <div
                        class="w-10 h-10 overflow-hidden transition-all border rounded-full bg-white/20 border-white/30 ring-2 ring-transparent hover:ring-secondary">
                        @if(($user->foto_profil ?? Auth::user()->foto_profil ?? null))
                            <img src="{{ asset('uploads/profile_photos/' . ($user->foto_profil ?? Auth::user()->foto_profil)) }}"
                                alt="Avatar" class="object-cover w-full h-full">
                        @else
                            <img src="https://ui-avatars.com/api/?name={{ urlencode($user->name ?? Auth::user()->name ?? 'User Name') }}&background=random"
                                alt="Avatar" class="object-cover w-full h-full">
                        @endif
                    </div>
                    <svg class="w-4 h-4 transition-transform"
                        :class="[open ? 'rotate-180' : '', scrolled ? 'text-gray-400' : 'text-white/80']" fill="none"
                    ...
                    <div class="py-1">
                        <div class="px-4 py-3 border-b border-gray-50">
                            <p class="text-sm font-bold leading-none text-gray-900">
                                {{ $user->name ?? Auth::user()->name ?? 'Username' }}</p>
                            <p class="mt-1 text-xs text-gray-500">{{ $user->email ?? Auth::user()->email ??
                                'user@example.com' }}</p>
                        </div>

                        <a href="{{ route('user.profile') }}"
                            class="flex items-center gap-2 px-4 py-2 text-sm text-gray-700 transition-colors hover:bg-primary/10 hover:text-primary">
                        ...
                        <a href="#"
                            class="flex items-center gap-2 px-4 py-2 text-sm text-gray-700 hover:bg-primary/10 hover:text-primary transition-colors">
                            <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
        ...
        <button @click="mobileOpen = !mobileOpen"
            class="p-2 text-2xl rounded-md lg:hidden focus:outline-none hover:bg-white/10"
        ...
        <div class="p-4 space-y-2">
            ...
            <a href="{{ route('user.profile') }}" @click="mobileOpen=false">
                <div class="flex items-center gap-3 px-4 py-3 mb-4 border border-gray-100 bg-gray-50 rounded-xl">
                    ...
                    <div>
                        <p class="text-sm font-bold text-gray-800">Hi,
                            {{ $user->name ?? Auth::user()->name ?? 'Username' }}</p>
                        <p class="text-xs text-gray-500">{{ $user->email ?? Auth::user()->email ?? 'user@example.com' }}
                        </p>
                    </div>
                </div>
            </a>
            ...
            <a href="#"
                class="block px-4 py-3 font-medium text-gray-700 rounded-xl hover:bg-primary/10 hover:text-primary">
                Logout
            </a>
        </div>
    </div>
