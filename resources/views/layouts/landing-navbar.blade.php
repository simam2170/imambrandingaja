<nav x-data="{ scrolled: false }" 
     ...
     :class="scrolled ? 'bg-white shadow-sm py-3 border-gray-200' : 'bg-[#08b3ad] py-4'"
>
    ...
    <div class="flex items-center justify-between px-4 mx-auto max-w-7xl sm:px-6 lg:px-8">
        ...
        <a href="{{ route('beranda') }}" class="flex items-center gap-2">
            <span class="text-2xl font-bold tracking-tight" :class="scrolled ? 'text-gray-900' : 'text-white'">
                Branding<span class="italic" :class="scrolled ? 'text-yellow-500' : 'text-yellow-400'">Aja</span><span :class="scrolled ? 'text-[#08b3ad]' : 'text-white/90'">.</span>
            </span>
        </a>
        ...
        <div class="items-center hidden gap-8 lg:flex">
            <ul class="flex items-center gap-6 font-medium" :class="scrolled ? 'text-gray-500' : 'text-white/90'">
                <li>
                    <a href="{{ route('beranda') }}"
                        class="transition-colors hover:text-yellow-400 font-bold"
                    >
                        Beranda
                    </a>
                </li>
            ...
            <div class="flex items-center gap-3">
                <a href="{{ route('bypass.user') }}" class="px-4 py-2 text-sm font-bold text-[#08b3ad] bg-white rounded-full hover:bg-gray-100 transition shadow-sm">
                    Masuk User
                </a>
                <a href="{{ route('bypass.mitra') }}" class="px-4 py-2 text-sm font-bold text-white border border-white rounded-full hover:bg-white/10 transition">
                    Masuk Mitra
                </a>
                 <a href="{{ route('bypass.admin') }}" class="px-4 py-2 text-sm font-bold text-gray-700 bg-gray-200 rounded-full hover:bg-gray-300 transition">
                    Admin
                </a>
            </div>
        </div>
