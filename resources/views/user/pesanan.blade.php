            ...
            <div class="flex flex-col gap-4 md:flex-row">
                ...
                <div class="flex gap-2">
                    <select
                        ...
                        <option>Urutkan Terbarru</option>
                ...
                <div class="max-h-[600px] overflow-y-auto pr-2 custom-scrollbar">
                    ...
                    @foreach (['menunggu_pembayaran', 'direview', 'diproses', 'selesai', 'ditolak', 'dibatalkan'] as $status)
                        <div x-show="activeStatus === '{{ $status }}'" class="space-y-4" @if($status !== 'menunggu_pembayaran') style="display: none;" @endif>
                            @forelse($orders->get($status, []) as $order)
                                <div class="relative flex items-center gap-6 p-6 overflow-hidden transition-all bg-white border border-gray-100 shadow-sm cursor-pointer group rounded-xl hover:shadow-md hover:border-primary/50">
                                    <div class="flex flex-col gap-4 sm:flex-row w-full">
                                        <div class="w-24 h-24 overflow-hidden bg-gray-100 rounded-lg shrink-0 flex items-center justify-center">
                                            <img src="https://ui-avatars.com/api/?name={{ urlencode($order->layanan->nama_layanan ?? 'Service') }}&background=f3f4f6&color=666" class="w-full h-full object-cover">
                                        </div>
                                        <div class="flex flex-col justify-between flex-1">
                                            <div>
                                                ...
                                                <p class="mt-2 text-sm text-gray-500">Jumlah: 1</p>
                                            </div>
                                            <div class="flex flex-col justify-between gap-4 mt-3 sm:flex-row sm:items-end">
                                                <div class="text-sm text-gray-500">No. Pesanan : <span class="font-bold text-gray-700">#{{ $order->order_number }}</span></div>
                            ...
                            @empty
                                <div class="flex flex-col items-center justify-center p-12 text-center bg-white/50 rounded-xl border border-dashed border-yellow-300">
                                    <div class="w-16 h-16 bg-white rounded-full flex items-center justify-center mb-4 text-yellow-400 shadow-sm">
                                        <svg xmlns="http://www.w3.org/2000/svg" class="h-8 w-8" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M20 13V6a2 2 0 00-2-2H6a2 2 0 00-2 2v7m16 0v5a2 2 0 01-2 2H6a2 2 0 01-2-2v-5m16 0h-2.586a1 1 0 00-.707.293l-2.414 2.414a1 1 0 01-.707.293h-3.172a1 1 0 01-.707.293l-2.414-2.414A1 1 0 006.586 13H4" />
                                        </svg>
                                    </div>
                                    <p class="text-gray-500 font-medium">Belum ada pesanan dengan status "{{ ucfirst($status) }}"</p>
                                </div>
                            @endforelse
                        </div>
                    @endforeach
