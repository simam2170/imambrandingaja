...
@section('content')
    <div class="max-w-4xl mx-auto space-y-8" x-data="{
        ...
        {{-- SIMPLIFIED VIEW FOR UPLOADED PROOF --}}
        <template x-if="hasProof && order.status !== 'dibatalkan' && order.status !== 'selesai' && order.status !== 'ditolak'">
            <div class="flex flex-col items-center justify-center min-h-[60vh] text-center">
                <div class="w-full max-w-2xl p-12 bg-white border border-blue-100 shadow-xl rounded-3xl">
                    ...
                    <div class="mt-8">
                        <a href="{{ route('user.pesanan') }}" class="inline-flex items-center justify-center px-8 py-3 text-sm font-bold text-white transition-all bg-blue-600 rounded-xl hover:bg-blue-700 shadow-lg shadow-blue-200">
                            Lihat Pesanan Saya
                        </a>
                    </div>
                </div>
        ...
        <template x-if="!hasProof || order.status === 'dibatalkan' || order.status === 'selesai' || order.status === 'ditolak'">
            <div>
                ...
                <div class="p-8 mb-8 bg-white border border-gray-100 shadow-sm rounded-3xl text-center">
                    ...
                    <div class="pt-4 space-y-4 border-t border-yellow-200" x-show="!hasProof && order.status !== 'dibatalkan'">
                                ...
                                <template x-for="item in order.items" :key="item.id">
                                    <div class="flex items-center gap-4 p-4 border bg-gray-50/50 rounded-2xl border-gray-50">
                                        ...
                                        <div class="flex-1 min-w-0">
                                            <h4 class="mb-1 text-sm font-bold text-gray-800 truncate" x-text="item.layanan?.nama_layanan || 'Layanan'"></h4>
                            ...
                            <div class="space-y-4">
                                ...
                                <template x-if="['bca', 'mandiri'].includes(order.metode_pembayaran)">
                                    <div class="p-4 bg-white border border-yellow-200 rounded-2xl">
                                        <p class="text-[10px] font-bold text-gray-400 uppercase mb-1" x-text="order.metode_pembayaran"></p>
                                        <p class="mb-1 text-sm font-black text-gray-800" x-text="'Bank ' + order.metode_pembayaran.toUpperCase()"></p>
                                        <div class="flex items-center justify-between px-2 py-1 font-mono text-lg font-bold text-yellow-600 rounded-lg bg-yellow-50">
                                            <span x-text="order.metode_pembayaran === 'bca' ? '123 456 7890' : '987 654 3210'"></span>
                                ...
                                <template x-if="['qris', 'dana'].includes(order.metode_pembayaran)">
                                    <div class="p-4 bg-white border border-yellow-200 rounded-2xl text-center">
                                        <p class="text-[10px] font-bold text-gray-400 uppercase mb-3 text-left" x-text="order.metode_pembayaran"></p>
                                ...
                                {{-- SHOW ONLY IF NO PROOF YET AND STATUS IS MENUNGGU PEMBAYARAN OR DIREVIEW --}}
                                <template x-if="!hasProof && (order.status === 'menunggu_pembayaran' || order.status === 'direview')">
                                    <div class="space-y-6">
                                ...
                                <template x-if="(order.status === 'menunggu_pembayaran' || order.status === 'direview') && !hasProof">
