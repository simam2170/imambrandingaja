                        ...
                        <div class="p-6">
                            ...
                            <div class="mt-6 pt-4 border-t border-gray-50 flex items-center justify-between">
                                ...
                                <div class="flex items-center gap-2" @click.stop>
                                    ...
                                    <button @click="services = services.filter(s => s.id !== item.id)"
                    ...
                    <div class="flex flex-col h-full overflow-hidden">
                        <div class="relative h-56 flex-shrink-0">
                            ...
                            <button @click="openDetailModal = false"
                                class="absolute top-4 right-4 p-2 bg-white/20 backdrop-blur-md rounded-full text-white hover:bg-white/40 transition-all">
                                <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="none" viewBox="0 0 24 24"
                                    stroke="currentColor">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                        d="M6 18L18 6M6 6l12 12" />
                            ...
                            <div class="absolute bottom-4 left-6 right-6">
                                <div class="flex items-center gap-2 mb-2">
                                    ...
                                    <span
                                        class="px-2.5 py-1 bg-green-100 text-green-600 text-[10px] font-bold uppercase rounded-full"
                                        x-text="selectedService.status"></span>
                                </div>
                        ...
                        <div class="p-8 overflow-y-auto flex-1 space-y-6">
                            <p class="text-gray-500 leading-relaxed" x-text="selectedService.deskripsi"></p>

                            {{-- INFO TAGS --}}
                            <template x-if="getServiceInfoEntries(selectedService).length > 0">
                                <div class="space-y-4">
                                    <template x-for="([key, values]) in getServiceInfoEntries(selectedService)" :key="key">
                                        <div>
                                            ...
                                            <div class="flex flex-wrap gap-1.5">
                                                <template x-for="val in values" :key="val">
                                                    <span
                                                        class="px-3 py-1 rounded-full text-xs font-semibold bg-primary/10 text-primary border border-primary/20"
                                                        x-text="val"></span>
                                                </template>
                                            </div>
                                        </div>
                            ...
                            {{-- TIPE + HARGA --}}
                            <template x-if="getServiceTipe(selectedService).length > 0">
                                <div>
                                    <h3 class="font-bold text-gray-800 mb-3 flex items-center gap-2 text-sm">
                                        ...
                                        <span x-text="getTipeLabel(selectedService.klasifikasi) + ' & Harga'"></span>
                                    </h3>
                                    <div class="space-y-2">
                                        <template x-for="tipe in getServiceTipe(selectedService)" :key="tipe.nama">
                                            <div
                                                class="p-4 rounded-2xl border border-gray-100 bg-gray-50/50 hover:border-primary/30 transition-all flex items-center justify-between">
                                                <span class="font-semibold text-gray-800 text-sm" x-text="tipe.nama"></span>
                                                <span class="font-black text-primary text-sm"
                                                    x-text="formatRupiah(tipe.harga)"></span>
                                            </div>
                                        </template>
                                    </div>
                                </div>
                        ...
                        <div class="p-8 border-t border-gray-50 flex gap-4 flex-shrink-0">
                            <button @click="openDetailModal = false"
                                class="flex-1 py-4 bg-gray-100 text-gray-600 font-bold rounded-2xl hover:bg-gray-200 transition-all">Tutup</button>
                            <button
                                class="flex-1 py-4 bg-primary text-white font-bold rounded-2xl shadow-lg shadow-primary/20 hover:bg-primary/90 transition-all flex items-center justify-center gap-2">
                                <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24"
                                    stroke="currentColor">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                        d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z" />
